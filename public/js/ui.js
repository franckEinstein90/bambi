...
const pageContainer = (function() {
            ...
            let calendar = new events.Registrar(),
                evRegExp = /Event from (\d{4}\-\d{2}\-\d{2}) to (\d{4}\-\d{2}\-\d{2})\:\s(.+)/,
                ...
                processDescription = (calendar, evInfo) => {
                    try {
                        let {
                            beginDate,
                            endDate,
                            title,
                            description
                        } = extractFields(evInfo); 
                        calendar.register(
                                new eventUtils.CalendarEvent(
                                    beginDate,
                                    endDate,
				                            title, 
                                    description));
                    } catch (e) {
                        throw e + "at pageContainer.processDescription"
                    }
              };

                    return {
                        onReady: (eventStrings) => {
                            eventStrings.forEach(str => processDescription(calendar, str));
